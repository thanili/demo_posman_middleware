...
public class CustomerPosRepositoryTest {
    @Autowired
    ...
    public void test_CustomerPosRepo_Save_FindById_Delete() {
        ...
        // Save the document
        customerPosRepository.save(customer);
        // Try to find the document by its ID
        Optional<CustomerLocal> foundCustomer = customerPosRepository.findById(customer.getCode());
        ...
        customerPosRepository.delete(customer);

        // Try to find the document again
        Optional<CustomerLocal> retrievedCustomer = customerPosRepository.findById(customer.getCode());
